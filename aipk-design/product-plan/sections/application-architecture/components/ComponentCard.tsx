...
export function ComponentCard({ component, onClick }: ComponentCardProps) {
  ...
  const handleClick = () => {
    ...
    navigate(`/sections/application-architecture/components/${component.id}`)
  }

  return (
    <button
      onClick={handleClick}
      className={`w-full flex items-start gap-4 rounded-lg border-2 bg-white p-4 text-left transition-all hover:shadow-md dark:bg-slate-800 ${borderColor}`}
    >
