...
It is similar to "design-os" which is focussed on front end (example of that is in the folder `aipk-design`). using prompts in coding agents files are created that are displayed in all sections. interaction is always in the coding agent and the UI is only readonly.
